...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-2ee5ce2b";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-2ee5ce2b">'+'<span class="tile-face-2ee5ce2b tile-front-2ee5ce2b"></span>'+'<span class="tile-face-2ee5ce2b tile-back-2ee5ce2b"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-2ee5ce2b")){return;}
var back=tile.querySelector(".tile-back-2ee5ce2b");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-2ee5ce2b");if(isMine){tile.classList.add("boom-2ee5ce2b");}
if(dim){tile.classList.add("dimmed-2ee5ce2b");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-2ee5ce2b")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-2ee5ce2b");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-2ee5ce2b")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-2ee5ce2b");void panelEl.offsetWidth;panelEl.classList.add("jolt-2ee5ce2b");}
endRound();}
